 ...
 * @author japarejo
 */
public class LoopUnfoldingExecutionPathsBuilder implements ExecutionPathsBuilder {
    ...
    private void buildPaths(List<AbstractWebService> previous, List<StructuralComponent> components, int i, double d, Set<ExecutionPath> execPaths,QoSAwareWSCompositionProblem aThis) {
        if(i<components.size())
        {
            ...
            }else if(comp instanceof Sequence)
            {
                ...
                List<StructuralComponent> newComponents=createComponents(subComponents,components,1,i);
                ...
                for(int j=0;j<=((Loop)comp).getAverageNumberOfIterations();j++){
                    newComponents=createComponents(subComponents,components,j,i);
                    buildPaths(previous,newComponents,i,d,execPaths,aThis);
                }
    ...
    private List<StructuralComponent> createComponents(List<StructuralComponent> subComponents, List<StructuralComponent> components, int j, int i) {
